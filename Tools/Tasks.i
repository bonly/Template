...
{
    if(idx >= get_tasks()->page_count || idx < 0 )
        return 0;
    return &list[idx];
}
...
{
    if (get_tasks()->page_count < 1)
      return;
    switch(get_tasks()->operation)
    {
      case NEXT_PAGE:
      {
        if (get_tasks()->get_page(get_tasks()->current_page)->page != 0)
        {
            get_tasks()->destory_task(get_tasks()->current_page);
            get_tasks()->get_page(get_tasks()->current_page)->page = 0;
        }
        if (get_tasks()->next_page == -1) 
        {
            
            destroyApp(false);
        }
        get_tasks()->current_page = get_tasks()->next_page; 
        get_tasks()->operation = CURRENT_PAGE;
      }
      case CURRENT_PAGE:
      {
        if (get_tasks()->get_page(get_tasks()->current_page)->creatFcn != 0)
        {
          if (get_tasks()->get_page(get_tasks()->current_page)->page == 0)
          {
            
            
            get_tasks()->get_page(get_tasks()->current_page)->page = get_tasks()->get_page(get_tasks()->current_page)->creatFcn();
            get_tasks()->get_page(get_tasks()->current_page)->page->init();
            get_tasks()->current_page = get_tasks()->get_page(get_tasks()->current_page)->idx;
          }
          get_tasks()->get_page(get_tasks()->current_page)->page->running();
        }
        break;
      }
    }
...
static void onPaint(void *p)
{
  ...
  if (get_tasks()->get_page(get_tasks()->current_page)->page != 0)
  {
    get_tasks()->get_page(get_tasks()->current_page)->page->onPaint();
  }
  return ;
}
...
static int init()
{
    ...
    {
        list[tasks->page_count].idx = tasks->page_count;
        ++tasks->page_count;
    }
